...
/**
 * Enhanced Sonar Digest Generator
 * 
 ...
 * It ensures all content is real, verified, and properly cited.
 * 
 * Key features:
 * 1. Uses Sonar deep research as the primary source
 * 2. Synthesizes with Grok deep research for additional context
 * 3. Implements advanced scoring to select the 5 most important stories
...
console.log('=== ENHANCED SONAR DIGEST GENERATOR ===');
...
console.log('4. Verify all citations to ensure they\'re real and accessible');
...
async function verifyCitations(sonarDigest) {
  ...
  return urlAccessibility;
}

// Function to score and select the top 5 topics
function scoreAndSelectTopics(sonarDigest, grokDigest, urlAccessibility, twitterData) {
  ...
  sonarDigest.topics.forEach(topic => {
    ...
    const accessibleCitations = (topic.citations || []).filter(citation => {
      return urlAccessibility[citation.url] === true;
    });
    
    // Skip topics with no accessible citations
    if (accessibleCitations.length === 0) {
      console.log(`Skipping topic "${topic.title}" because it has no accessible citations`);
      return;
    }
    ...
    const overallScore = calculateOverallScore(topic, accessibleCitations, twitterImpactScore);
...
async function main() {
  try {
    ...
    const urlAccessibility = await verifyCitations(sonarDigest);
    
    // Step 5: Score and select the top 5 topics
    const topTopics = scoreAndSelectTopics(sonarDigest, grokDigest, urlAccessibility, twitterData);
