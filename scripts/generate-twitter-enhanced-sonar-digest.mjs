...
/**
 * This script generates a Twitter-enhanced Sonar digest by combining
 * the latest Sonar digest with Twitter data.
 * 
 * Run with: node scripts/generate-twitter-enhanced-sonar-digest.mjs
 */
...
async function main() {
  try {
    ...
    if (!fs.existsSync(sonarDigestPath)) {
      console.error('Sonar digest file not found. Please run the generate-sonar-digest.mjs script first.');
      process.exit(1);
    }
    ...
    if (!fs.existsSync(twitterDataPath)) {
      console.error('Twitter data file not found. Please run the fetch-real-twitter-data.js script first.');
      process.exit(1);
    }
    ...
    console.log('To view the enhanced digest, visit: http://localhost:3000/news/sonar-digest?source=twitter-enhanced');
